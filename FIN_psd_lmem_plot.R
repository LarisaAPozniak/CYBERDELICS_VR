...
sens <- subset(p_vals, rn == "hyper")
sens <- subset(sens, p_value < 0.05)
ses <- c(sens$sensor)
alpha_color <- subset(delta, sensor %in% ses)
alpha_color


print(ses)
ses = c('Fp1', 'Fz',  'F3',  'FC5', 'FC1','C3', 
        'CP5' ,'CP1', 'P7',  'O1',  'O2',  'CP6',
        'CP2', 'Cz' , 'T8',  'FC6', 'FC2', 'F4' , 'F8')
alpha_color <- subset(alpha, sensor %in% ses)

temp <-subset(alpha, sensor  %in% c( 'CP2','CP1',  'Pz', 'Cz', 'FC2', 'FC6', 'T8', 'F8', 'Fp2'))
m <- lmer(z_score ~ hyper*color + (1|subject), data = alpha_color)
marginal_em <- emmeans(m, ~ as.factor(hyper),level = 0.95,lmer.df = "satterthwaite")
marginal_em<-as.data.frame(marginal_em)
Tuk<-data.table(summary(emmeans(m, pairwise ~ hyper|color, adjust = 'tukey',lmer.df = "satterthwaite"))$contrasts)
...
write.csv(marginal_em, "emmean_results_delta_fk_3007.csv")
write.csv(Tuk, "TUK_results_delta_fk_3007.csv")

marginal_em <- marginal_em %>%
  mutate(Geometry = if_else(hyper %in% c("hyperbolic"), 'HoneyCombs', 'ControlCubes'))
...
print(ses)
ses = c('Fp1', 'Fz',  'F3',  'FC5', 'FC1','C3',  'CP5' ,'CP1', 'P7',  'O1',  'O2',  'CP6', 'CP2', 'Cz' , 'T8',  'FC6', 'FC2', 'F4' , 'F8')
alpha_color <- subset(alpha, sensor %in% ses)

m1 <- lmer(z_score ~ hyper*color + (1|subject), data = alpha_color)

#means within the model
marginal_em <- emmeans(m1, ~ as.factor(color),level = 0.95,lmer.df = "satterthwaite")
marginal_em<-as.data.frame(marginal_em)
...
#plot
plot <- ggplot(marginal_em, aes(x = color, y = emmean, color = color )) +#, group = hyper, color = hyper, fill = hyper)) +
  geom_errorbar(aes(ymin = emmean - SE, ymax = emmean + SE), 
                width = 0.5, position = position_dodge(width = 0.2), size = 2) +
  #geom_bar(binwidth = 5,aes(group = hyper, fill = hyper), stat = "identity")+
  geom_line(size = 2, aes(group = color), position = position_dodge(width = 0.2)) +
